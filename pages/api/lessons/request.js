...
export default async function handler(req, res) {
  ...
  try {
    const { userAddress, lessonData, lessonPaymentUnit = 'lovelace' } = req.body;
    if (!userAddress || !lessonData) {
        return res.status(400).json({ error: "User address and lesson data are required." });
    }

    // Fetch the lesson request script and its address
    const script = {
      type: "PlutusV3",
    ...
    const datum = buildTransactionDatum(lessonData, lockUnit, lockAmount, lessonPaymentUnit);
    if (!datum) {
