import { Lucid, Blockfrost, validatorToAddress, Data } from "@lucid-evolution/lucid";
...
const getUserLucid = async (address) => {
    ...
    const lucid = await Lucid( new Blockfrost(apiUrl, apiKey), process.env.NEXT_PUBLIC_BLOCKFROST_NETWORK );
...
function buildTransactionDatum(data) {
    try {
        return Data.to(data, Data.Object({
            student: Data.Bytes(),
            teacher: Data.Bytes(),
            admin: Data.Bytes(),
...
export default async function handler(req, res) {
    ...
    try {
        ...
        const validatorAddress = validatorToAddress(process.env.NEXT_PUBLIC_BLOCKFROST_NETWORK, script);
        ...
        let tx = await lucid.newTx()
            .pay.ToContract(validatorAddress, { kind: 'inline', value: datum }, amounts)
