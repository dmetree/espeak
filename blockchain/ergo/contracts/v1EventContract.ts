export const v1Contract = `
{
    ...
    // Name: MindHealer Event Contract
    // Description: Contract for the event box controlling how the client(s) and expert interact with each other during an event.
    //              An expert creates an event by depositing the event price.
    //              An expert can have full refund of the deposit and delete the event if there're no participants. 
    //              A client can join the event by paying the event price. 
    //              An expert can make a refund to client and remove client from event. 
    //              After the event is over, expert can claim rewards (all payments at once): 
    //              80% goes to expert
    //              12% goes to partner(s) layer one (if any, else to service)
    //              3% goes to partner(s) layer two (if any, else to service)  
    //              5% goes to service.
    //
    // Description: Contract for the event box controlling how the client(s) and expert interact with each other during an event. 
    //              A client can join the event by paying the event price. 
    //              An expert can make a refund to client and remove client from event. 
    //              After the event is over, expert can claim rewards: 
    //              80% goes to expert
    //              12% goes to partner layer one (if any, else to service)
    //              3% goes to partner layer two (if any, else to service)  
    //              5% goes to service.       
    // Version: 1.0.0
    // Author: Demian B. (demianbondarenko@gmail.com) 
    // Auditor: Luca D'Angelo (ldgaetano@protonmail.com)

    // ===== Box Contents ===== //
    // Tokens
    // 1. (EventSingletonId, 1)
    // 2. (SigUSDId, EventPrice + ?Collateral) // If provided by the expert.
    //
    // Registers
    // R4: Int                              eventStartTimeBlockHeight
    // R5: (SigmaProp, SigmaProp)           (clientAddressSigmaProp, expertAddressSigmaProp)            // Expert address is initially the client address before the session is accepted.
    // R6: (Coll[Byte], Coll[Byte])         (partnerLayerOneAddressBytes, partnerLayerTwoAddressBytes)  // Empty Coll[Byte]() if not present.
    // R7: (Boolean, Boolean)               (isEventAccepted) // Both false initially.
    // R8: Long                             eventPrice
    // R9: Long                             
    ...
    // Inputs: Event, Client
    // Data Inputs: None
    // Outputs: Event, Client,
    // Context Variables: TxType
    //
    // 2. Cancel Event Tx: Psychologist
    // Inputs: Event, PsychologistPK
    // Data Inputs: None
    // Outputs: Client
    // Context Variables: TxType
    //
    // 3. Event End Tx: No Problem
    // Inputs: Event, PsychologistPK
    // Data Inputs: None
    // Outputs: Expert, ?PartnerLayerOneFee, ?PartnerLayerTwoFee, PsyWorkshopFee
    // Context Variables: TxType
