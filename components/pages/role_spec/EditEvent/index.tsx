...
type EventForm = {
    ...
    image: string | null;
    author: Author;
    eventType: EventType; // ✅ now properly typed
    status: EventStatus;
    hours: number;
};
...
const EditEvent = () => {
    ...
    console.log("SE: ", selectedEventDetails)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const eventRef = doc(database, "events", selectedEventDetails.id);
            await updateDoc(eventRef, eventData);
            toast.success("Event updated!");
            resetForm();
            router.replace(`/event_details/${selectedEventDetails.id}`);
        } catch (error) {
    ...
    useEffect(() => {
        if (selectedEventDetails) {
            setFormData({
                language: selectedEventDetails.language || null,
                title: selectedEventDetails.title || "",
                description: selectedEventDetails.description || "",
                videoLink: selectedEventDetails.videoLink || "",
                price: selectedEventDetails.price || "",
                dates: selectedEventDetails.dates || [""],
                students: selectedEventDetails.students || [],
                image: selectedEventDetails.image || null,
                author: selectedEventDetails.author || {
                    uid: userUid,
                    avatar: userData?.avatar || "",
                    nickname: userData?.nickname || "",
                    rank: userData?.psyRank || "",
                },
                eventType: selectedEventDetails.eventType || EventType.OneTime,
                status: selectedEventDetails.status || EventStatus.InProcess,
                hours: selectedEventDetails.hours || 0,
            });
        }
    }, [selectedEventDetails, userUid, userData]);

    return (
        <Page className={s.userboardPage}>
            <Substrate className={s.userboard} color="bg-color">
                <h2 className={s.formStepH}>{t.edit_event}</h2>
