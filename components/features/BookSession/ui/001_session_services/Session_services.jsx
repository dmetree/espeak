...
export function SessionServices() {
  ...
  const selectedSpecialist = useSelector(
    ({ specialists }) => specialists.selectedSpecialist
  );

  const services =
    draftAppointment?.services || selectedSpecialist?.services || [];
      ...
      <div className={s.wrapper}>
        {!hasServices ? (
          <p className={s.noServices}>
            There are no available services, so you wouldn't be able to book a
            lesson from this teacher
          </p>
        ) : (
