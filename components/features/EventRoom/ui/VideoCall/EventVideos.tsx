...
export default function Page() {
  ...
  const eventRoomId = useSelector(({ events }) => events.selectedEvent.id);
  const eventRoom = useSelector(({ events }) => events.selectedEvent);
  const videoCall = useSelector(({ video }) => video.showVideoCall); // should I replace it with EventCall?
  const userData = useSelector(({ user }) => user?.userData);
  const myEvents = useSelector(({ events }) => events.myEvents); // get myEvents from Firebase
  ...
  const endSession = async () => {
    try {
      setToken('');
      dispatch(hideModal(EModalKind.EventRoom));
  ...
  const initiateVideoCall = async () => {
    try {
      ...
      at.addGrant({ room: eventRoomId, roomJoin: true, canPublish: true, canSubscribe: true });
